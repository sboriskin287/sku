...
namespace sku_to_smv
{
    ...
    public class cParser
    {
        ...
        public parceResult ParseStart(String InputString)
        {
            ...
            //Убираем пробелы, знаки табуляции и перехода на новую строку
    ...
    public class cSympleParser
    {
        enum state { H, COMMENT, SIGNAL };
                ...
                switch (CurrentState)
                {
                    ...
                    case state.SIGNAL: if (str[i] != ';' && str[i] != '(' && str[i] != ')' && str[i] != '&' && str[i] != '|' && str[i] != '~' && str[i] != '#')
                        ...
                        else
                        {
                            ...
                            else elements[currentElement].TextColor = Settings.Default.TextFieldSignalColor;
                            if (str[i] == '#')
                            {
                                Array.Resize(ref elements, elements.Length + 1);
                                currentElement++;
